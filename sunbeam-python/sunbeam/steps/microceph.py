...
class RemoveMicrocephOSDsStep(BaseStep):
    ...
    def _prepare(self) -> Result:
        ...
        if node_info is not None and Role.STORAGE.name.lower() not in node_info.get(
            "role", ""
        ):
        ...
        if not target_machine_ids:
            return Result(
                ResultType.FAILED,
                f"Unable to identify machine for MicroCeph node {self.node}",
            )
        ...
        unknown_machine = False
        for unit_name, unit in units.items():
            machine_id = getattr(unit, "machine", None)
            if machine_id is None:
                unknown_machine = True
                continue
        ...
        if unknown_machine or not (target_units or surviving_units):
            return Result(
                ResultType.FAILED,
                f"Unable to identify a MicroCeph unit for {self.node}",
            )
        if surviving_units:
            self.unit = sorted(surviving_units)[0]
        else:
            self.unit = sorted(target_units)[0]
    ...
    def _run_command(self, command: str) -> str:
        ...
        if result.return_code != 0:
            raise SunbeamException(
                f"Command {command!r} failed with return code "
    ...
    def _parse_configured_disks(stdout: Any) -> list[dict[str, Any]]:
        ...
        for disk in configured_disks:
            ...
            if not isinstance(disk["path"], str):
                raise ValueError("Configured disk has an invalid path")
            disks.append(disk)
        return disks
    ...
    def _list_configured_osd_ids(self) -> list[int]:
        ...
        try:
            disks = self._parse_configured_disks(
                self._run_command("microceph disk list --json")
            )
    ...
    def _list_crush_osd_ids(self) -> list[int]:
        """Return OSD IDs under the exact target CRUSH host."""
        try:
